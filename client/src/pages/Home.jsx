...
const Home = () => {
  ...
  useEffect(() => {
    const fetchOfferListings = async() => {
      try {
        const {data} = await axios('/api/listing/get?offer=true&limit=4');
    ...
    const fetchRentListing = async() => {
      try {
        const {data} = await axios('/api/listing/get?type=rent&limit=4');
    ...
    const fetchSaleListing = async() => {
      try {
        const {data} = await axios('/api/listing/get?type=sale&limit=4');
    ...
    <div>
      ...
      <div className="flex flex-col gap-6 p-28 px-3 max-w-6xl mx-auto">
        ...
        <div className='text-gray-400 text-xs sm:text-sm'>
          my property is the best place to find your next perfect place to live.
          <br />
      ...
      <Swiper navigation>
        ...
        offerListings.map((listing)=> (
          <SwiperSlide key={listing._id} >
            <div style={{background:`url(${listing?.imageUrl[0]}) center no-repeat`, backgroundSize:'cover'}} className='
            h-[500px]'>
